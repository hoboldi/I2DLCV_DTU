...
def train(args):
    ...
    train_dataset = FlowDataset(root_dir=args.root_dir, split=args.split,
                                transform=train_transform, n_sampled_frames=args.n_sampled_frames)
    train_loader = DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True,
                              num_workers=args.workers, pin_memory=True)

    val_loader = None
    if args.evaluate:
        val_dataset = FlowDataset(root_dir=args.root_dir, split=args.eval_split,
                                  transform=val_transform, n_sampled_frames=args.n_sampled_frames)
    ...
    model = Network(num_classes=args.num_classes, dropout_rate=args.dropout,
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # === CHANGED: evaluate BEFORE scheduler.step to get the monitored metric ===
        val_loss = None
        val_acc = None
        ...
        scheduler.step(metric_for_lr)  # Reduce on plateau based on chosen metric
        ...
        # best model selection (same logic as before)
        if args.evaluate and val_loader is not None:
        ...
        # Optional: show LR change summary
...
def parse_args():
    ...
    parser.add_argument('--in-channels', type=int, default=18, help='number of input channels for flow (e.g. 18)')
    parser.add_argument('--n-sampled-frames', type=int, default=10, help='number of frames sampled per video (used by FlowDataset)')
    ...
    # kept for backwards-compat (not used by ReduceLROnPlateau)
    ...
    # === NEW: plateau scheduler tuning knobs ===
